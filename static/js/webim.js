	...
	conn.getRoster({
		success : function(roster) {
			...
			if (bothRoster.length > 0) {
				...
				{
					setCurrentContact(defaultChatId);
				}
				else
				{
					curroster = bothRoster[0];
					setCurrentContact(curroster.name); 
				}
			}
			//获取当前登录人的群组列表
			conn.listRooms({
...
var appendMsg = function(who, contact, message) {
	...
	// console.log("当前的聊天窗口的用户Id为：" + contact);
	// console.log("当前的用户Id为：" + curChatUserId);
	// console.log(curChatUserId.indexOf(contact));
	if (curChatUserId && curChatUserId != contact) {
		var contactLi = getContactLi(contactDivId);
		if (contactLi == null) {
			return;
		}
		// console.log("背景马上应该变成绿色。");
...
// 			// });
// 			// curChatUserId = "77f9a16a-1387-11e6-a4b7-419c4f2cd8bd";
// 			// sendText();

// 			//后回调，这里还是null
// 			// console.log((curChatUserId));
// 			//name......
// 			// curChatUserId = "root";
// 			// chatUserId = "root";
// 			// 传这个
